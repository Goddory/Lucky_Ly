...
namespace FlutterUnityIntegration.Editor
{
    public class Build : EditorWindow
    {
        ...
        private static readonly string AndroidExportPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/android/unityLibrary"));
        private static readonly string WindowsExportPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/windows/unityLibrary/data"));
        private static readonly string IOSExportPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/ios/UnityLibrary"));
        private static readonly string WebExportPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/web/UnityLibrary"));
        private static readonly string IOSExportPluginPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/ios_xcode/UnityLibrary"));
...
#unity-mobile-warning { position: absolute; left: 50%; top: 5%; transform: translate(-50%); background: white; padding: 10px; display: none }
        ...
        private static void BuildIOS(String path, bool isReleaseBuild)
        {
            ...
            #if UNITY_2022_1_OR_NEWER
                PlayerSettings.SetIl2CppCompilerConfiguration(UnityEditor.Build.NamedBuildTarget.iOS, isReleaseBuild ? Il2CppCompilerConfiguration.Release : Il2CppCompilerConfiguration.Debug);
        ...
        */
        ...
        {
            var androidPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/android"));
            var androidAppPath = Path.GetFullPath(Path.Combine(ProjectPath, "../../backend/lucky_ly_mobile/android/app"));
            
            string projBuildPath = Path.Combine(androidPath, "build.gradle");
            bool isProjKotlin = false;
            if (!File.Exists(projBuildPath)) {
                projBuildPath = Path.Combine(androidPath, "build.gradle.kts");
                isProjKotlin = true;
            }
            
            string appBuildPath = Path.Combine(androidAppPath, "build.gradle");
            bool isAppKotlin = false;
            if (!File.Exists(appBuildPath)) {
                appBuildPath = Path.Combine(androidAppPath, "build.gradle.kts");
                isAppKotlin = true;
            }

            string settingsPath = Path.Combine(androidPath, "settings.gradle");
            bool isSettingsKotlin = false;
            if (!File.Exists(settingsPath)) {
                settingsPath = Path.Combine(androidPath, "settings.gradle.kts");
                isSettingsKotlin = true;
            }

            if (!File.Exists(projBuildPath)) return;
            var projBuildScript = File.ReadAllText(projBuildPath);
            var settingsScript = File.Exists(settingsPath) ? File.ReadAllText(settingsPath) : "";
            var appBuildScript = File.Exists(appBuildPath) ? File.ReadAllText(appBuildPath) : "";
            ...
            if (!Regex.IsMatch(projBuildScript, @"flatDir[^/]*[^}]*}"))
            {
                ...
                string flatDirSnippet = isProjKotlin ? 
                    "        flatDir {\n            dirs(\"${project(\":unityLibrary\").projectDir}/libs\")\n        }" :
                    "        flatDir {\n            dirs \"${project(':unityLibrary').projectDir}/libs\"\n        }";

                projBuildScript = regex.Replace(projBuildScript, "allprojects {\n    repositories {\n" + flatDirSnippet + "\n");
                File.WriteAllText(projBuildPath, projBuildScript);
            }

            // Sets up the project settings.gradle files correctly
            string settingsRegex = isSettingsKotlin ? @"include\("":unityLibrary""\)" : @"include "":unityLibrary""";
            if (!Regex.IsMatch(settingsScript, settingsRegex))
            {
                if (isSettingsKotlin) {
            ...
            string implRegex = isAppKotlin ? @"implementation\(project\("":unityLibrary""\)\)" : @"implementation project\(':unityLibrary'\)";
            if (!Regex.IsMatch(appBuildScript, @"dependencies \{"))
            {
                string implSnippet = isAppKotlin ? "    implementation(project(\":unityLibrary\"))" : "    implementation project(':unityLibrary')";
                appBuildScript += "\ndependencies {\n" + implSnippet + "\n}\n";
                File.WriteAllText(appBuildPath, appBuildScript);
            } else if (!Regex.IsMatch(appBuildScript, implRegex))
            {
                var regex = new Regex(@"dependencies \{", RegexOptions.Multiline);
                string implSnippet = isAppKotlin ? "    implementation(project(\":unityLibrary\"))" : "    implementation project(':unityLibrary')";
                appBuildScript = regex.Replace(appBuildScript, "dependencies {\n" + implSnippet + "\n");
                File.WriteAllText(appBuildPath, appBuildScript);
            }
        }
